...
function displayFavorites(response){
  var city = response['data']['attributes']['fav_cities_weather'][0]['city'];
  var summary = response['data']['attributes']['fav_cities_weather'][0]['current_weather'][0]['current_summary'];
  var current_temp = response['data']['attributes']['fav_cities_weather'][0]['current_weather'][0]['current_temp'];
  var humidity = response['data']['attributes']['fav_cities_weather'][0]['current_weather'][0]['humidity'];
  var low = response['data']['attributes']['fav_cities_weather'][0]['current_weather'][0]['low'];
  var high = response['data']['attributes']['fav_cities_weather'][0]['current_weather'][0]['high'];

  var fav_data = '<h2>' + city + '</h2>' +
                  summary +
                  '<h3>' + current_temp + '&deg' + '</h3>' +
                  'Humidity: ' + humidity +
                  '<h4>' + low + '&deg' + '/' + high + '&deg' + '</h4>';
  document.getElementById('favorited_cities').innerHTML = fav_data;
}
